...
int main(int argc, char* argv[]){
 int q, n, i,j,k=1,aux=0,l=1; 
 while (1){
  scanf_s("%d", &n);
  scanf_s("%d", &q);
  if (n == 0 && q == 0 )
   break;
  int *res = calloc(q, sizeof(int));//respostas depois de ordenado
  int *c = calloc(q, sizeof(int));// chaves de busca
  int *vet = calloc(n, sizeof(int));//marmores
  for (i = 0; i < n; i++){
   scanf_s("%d", &vet[i]);
  }
  for (i = 0; i <q; i++){
